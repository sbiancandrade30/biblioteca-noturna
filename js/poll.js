...
function formatPollKey(value) { const [year, monthNumber] = value.split("-").map(Number); return formatter.format(new Date(year, monthNumber - 1, 1)).replace(/^(.)/, (_, character) => character.toUpperCase()); }
...
function subscribeToPoll() {
  ...
  month = pollDate();
  unsubscribe = onSnapshot(collection(db, "polls", pollId(), "responses"), snapshot => {
...
qs("#saveButton").onclick = async () => {
  if (!firebaseReady || !db) { qs("#saveMessage").textContent = "A votação online ainda não está conectada. Confira a configuração do Firebase e tente novamente."; return; }
...
qs("#newPollButton").onclick = () => { qs("#newPollMonth").value = pollKey; qs("#newPollDialog").showModal(); };
qs("#closeNewPollDialog").onclick = () => qs("#newPollDialog").close();
qs("#newPollDialog").addEventListener("click", event => { if (event.target === event.currentTarget) event.currentTarget.close(); });
qs("#createPollButton").onclick = () => { const nextPollKey = qs("#newPollMonth").value; if (!nextPollKey) return; qs("#newPollDialog").close(); setPoll(nextPollKey); };
...
async function initializeFirebase() {
  ...
  try { const app = initializeApp(config); db = getFirestore(app); await signInAnonymously(getAuth(app)); firebaseReady = true; subscribeToPoll(); }
  catch (error) { console.error(error); qs("#saveMessage").textContent = "A votação online não pôde ser conectada."; }
}
updatePollSelect(); refresh(); initializeFirebase();
